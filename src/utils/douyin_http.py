...
try:
    ...
    ABogus = None  # signing not available in public version
...
async def _harvest_tokens_from_cdp(port: int = 9222) -> dict:
    """從 CDP Chrome 收割 msToken / uifid / verifyFp / webid。

    通過在真實瀏覽器中執行一次搜索，截取請求參數中嘅 token。
    """
    tokens = {"msToken": "", "uifid": "", "verifyFp": "", "webid": ""}
    try:
        ...
        await page.goto("https://www.douyin.com/search/Python?type=general",
                        wait_until="domcontentloaded", timeout=15000)
        await page.wait_for_timeout(5000)

        if captured:
            p = captured[0]
            tokens["msToken"] = p.get("msToken", "")
            tokens["uifid"] = p.get("uifid", "")
            tokens["verifyFp"] = p.get("verifyFp", p.get("fp", ""))
            tokens["webid"] = p.get("webid", "")
            logger.info(f"Harvested msToken={tokens['msToken'][:20]}... uifid={tokens['uifid'][:20]}...")

        await page.close()
        await playwright.stop()
    except Exception as e:
        logger.warning(f"Harvest tokens from CDP failed: {e}")

    return tokens


async def search(keyword: str, count: int = 20, offset: int = 0) -> list[dict]:
    """抖音搜索 — 經 CDP Chrome 發請求（真實 TLS 指紋，避免 verify_check）。

    通過 Playwright 連接已登入嘅 CDP Chrome，打開搜索頁面，
    截取 API 響應直接提取數據。因為用真實 Chrome 嘅 TLS 指紋，
    所以唔會有 httpx 嘅 verify_check 問題。
    """
    _ = _get_best_session()
    ...
    try:
        ...
        async def on_response(response):
            if 'search/single/' in response.url and 'aweme/v1/web/general' in response.url:
                try:
                    body = await response.json()
                    if body.get("status_code") == 0 and body.get("data"):
        ...
        await page.wait_for_timeout(8000)  # 等 JavaScript 加載完成 + API 請求發出
        ...
        logger.error(f"Douyin CDP search failed: {e}")
        return []
