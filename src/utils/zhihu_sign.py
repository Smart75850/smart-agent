"""知乎 x-zse-96 签名 — 纯 Python 移植自 MediaCrawler zhihu.js (SM4 风格分组密码)。"""
import hashlib
import random
import re
import struct
...
_ZK = [1170614578, 1024848638, 1413669199, -343334464, -766094290, -1373058082,
...
_ZB = [20, 223, 245, 7, 248, 2, 194, 209, 87, 6, 227, 253, 240, 128, 222, 91,
...
def _i(val: int, arr: list, offset: int):
    """Write 32-bit int as 4 bytes big-endian."""
...
def _B(arr: list, offset: int) -> int:
    """Read 4 bytes as 32-bit int big-endian."""
    v = ((255 & arr[offset]) << 24 | (255 & arr[offset + 1]) << 16 |
         (255 & arr[offset + 2]) << 8 | 255 & arr[offset + 3])
    return v if v < 0x80000000 else v - 0x100000000


def _G(val: int) -> int:
    """S-box substitution + linear transform."""
    t = [0] * 4
    n = [0] * 4
    _i(val, t, 0)
    n[0] = _ZB[255 & t[0]]
...
def _encode_0_16(arr: list) -> list:
    """Encode first 16 bytes with secret offset and XOR 42."""
...
def _get_init_array(md5_hex: str) -> list:
    """Build 48-byte initialization array from MD5 hex."""
...
def _get_zse_96(md5_hex: str) -> str:
    """Compute x-zse-96 signature from MD5 hex."""
