...
class KuaishouAdapter(JsonAdapterMixin, PlatformAdapter):
    @property
    def name(self) -> str:
        return "kuaishou"
    ...
    async def search(self, keyword: str, limit: Optional[int] = None,
        ...
        return self._unwrap(await kuaishou_search(keyword, count=limit or 40), limit)

    async def hot(self, limit: Optional[int] = None) -> list[dict]:
        return self._unwrap(await kuaishou_hot(), limit)

    async def detail(self, item_id: str, **kwargs) -> dict:
        return self._unwrap_dict(await kuaishou_detail(item_id))

    async def comment(self, item_id: str, limit: Optional[int] = None) -> list[dict]:
        return self._unwrap(await kuaishou_comment(item_id), limit)

    async def user(self, user_id: str, limit: Optional[int] = None) -> list[dict]:
        return self._unwrap(await kuaishou_user(user_id), limit)
